# Email Templates

Copy-paste templates for common referral program communications.

---

## Referrer Applications

### Referrer Approved {#referrer-approved}

**Subject:** Welcome to Aruna Talent's Referral Program!

```
Hi [NAME],

Great news - your application to join our referral program has been approved!

Here's everything you need to get started:

YOUR REFERRER CODE: [CODE]

YOUR DASHBOARD:
https://my-referrals.html?code=[CODE]
(Bookmark this - it shows your referrals, earnings, and stats)

YOUR REFERRAL LINKS:
• For potential creators: apply.arunatalent.com?ref=[CODE]
• To recruit other referrers: refer.arunatalent.com?ref=[CODE]

HOW IT WORKS:
1. Share your creator link with people who might want to join our agency
2. When they sign up and start earning, you earn 5% of their revenue for the first 3 months, then 2.5%+ lifetime based on your tier
3. Track everything in real-time on your dashboard

TIPS FOR SUCCESS:
• Focus on people you know personally - warm referrals convert best
• Look for people already creating content or interested in starting
• Quality over quantity - one great creator beats 10 uncommitted ones

Questions? Just reply to this email.

Welcome aboard!

[YOUR NAME]
Aruna Talent
```

---

### Referrer Rejected {#referrer-rejected}

**Subject:** Your Aruna Talent Referral Application

```
Hi [NAME],

Thank you for your interest in joining our referral program.

After reviewing your application, we've decided not to move forward at this time. This could be due to:
• Limited network access to potential creators
• Misalignment with our current program focus
• Application information that didn't meet our criteria

This isn't necessarily permanent - our needs change over time. You're welcome to reapply in 6 months if your situation changes.

Thank you for understanding.

Best regards,
[YOUR NAME]
Aruna Talent
```

---

### Referrer More Info Needed {#referrer-more-info}

**Subject:** Quick Question About Your Referral Application

```
Hi [NAME],

Thanks for applying to our referral program! Before we can make a decision, we need a bit more information:

[SPECIFIC QUESTIONS - customize as needed]
• Could you tell us more about your connection to potential creators?
• What's your Instagram handle so we can check out your profile?
• How did you hear about our program?

Just reply to this email with the details and we'll get back to you quickly.

Thanks!
[YOUR NAME]
Aruna Talent
```

---

### Referrer Welcome (Auto-sent) {#referrer-welcome}

**Subject:** Your Aruna Talent Referral Links Are Ready!

```
Hi [NAME],

Welcome to the Aruna Talent referral program!

YOUR CODE: [CODE]

QUICK LINKS:
📊 Your Dashboard: my-referrals.html?code=[CODE]
🔗 Creator Referral Link: apply.arunatalent.com?ref=[CODE]
👥 Referrer Recruitment Link: refer.arunatalent.com?ref=[CODE]

You earn 5% of every creator's revenue for their first 3 months, then your tier rate (2.5%+) lifetime. Higher volume = higher rate.

Start sharing your links and watch your earnings grow!

- The Aruna Talent Team
```

---

## Creator Communications

### Direct Referral Outreach {#direct-referral-outreach}

**Subject:** [REFERRER NAME] Recommended You

```
Hi [NAME],

[REFERRER NAME] thought you'd be a perfect fit for Aruna Talent and passed along your info.

We're a talent agency that helps creators earn significant income through streaming and content creation. Our top creators earn $50k-$150k+ per month.

What we offer:
• Full support and training
• Marketing and growth strategies
• Technical setup assistance
• 24/7 support team

Interested in learning more? You can:
• Apply directly: apply.arunatalent.com
• Reply to this email with questions
• Schedule a quick call: [CALENDLY LINK if available]

No pressure - just wanted to reach out since [REFERRER NAME] spoke so highly of you.

Best,
[YOUR NAME]
Aruna Talent
```

---

### Direct Referral Follow-up {#direct-referral-followup}

**Subject:** Following up - Aruna Talent

```
Hi [NAME],

Just following up on my email from a few days ago. [REFERRER NAME] recommended you for our talent program.

If you have any questions about how it works or want to chat, I'm happy to help. Just reply to this email.

If this isn't for you, no worries at all - just let me know and I won't follow up again.

Best,
[YOUR NAME]
Aruna Talent
```

---

## Referrer Notifications

### New Model Added {#referrer-new-model}

**Subject:** Great News - New Creator Added to Your Account!

```
Hi [REFERRER NAME],

Exciting update - [MODEL NAME] has officially started with Aruna Talent and has been added to your referral account!

What this means:
• You'll earn 5% of [MODEL NAME]'s revenue for their first 3 months, then your tier rate (2.5%+) lifetime
• Track their earnings on your dashboard: my-referrals.html?code=[CODE]
• Commissions are calculated and paid monthly

Keep up the great work with your referrals!

- The Aruna Talent Team
```

---

### Monthly Revenue Updated {#monthly-revenue-updated}

**Subject:** [MONTH] Earnings Updated - Check Your Dashboard

```
Hi [REFERRER NAME],

Your [MONTH] earnings have been updated!

Log in to your dashboard to see:
• Each creator's monthly revenue
• Your commission earnings
• Lifetime totals

View Dashboard: my-referrals.html?code=[CODE]

Questions about your earnings? Just reply to this email.

- The Aruna Talent Team
```

---

## Internal Reminders

### Monthly Revenue Reminder {#monthly-revenue-reminder}

**Subject:** ACTION REQUIRED: Update Monthly Revenue

```
Hi Team,

It's the 1st of the month - time to update creator revenue in the Google Sheet.

STEPS:
1. Pull revenue data from payment platform for last month
2. Open Models tab: [SHEET LINK]
3. Update Column G (monthlyRevenue) for each active model
4. Format: Mon YYYY:amount (e.g., Jan 2026:96341.67)

DEADLINE: Complete by the 5th

Full SOP: docs/sops/04-monthly-revenue.md

- Automation Bot
```

---

### Weekly Summary {#weekly-summary}

**Subject:** Weekly Referral Program Summary - [DATE RANGE]

```
WEEKLY SUMMARY: [DATE RANGE]

NEW APPLICATIONS:
• Referrer Applications: [#]
• Creator Applications: [#]
• Direct Referrals: [#]

PENDING ITEMS:
• Applications awaiting review: [#]
• Direct referrals to contact: [#]

NEW THIS WEEK:
• New referrers added: [LIST]
• New models started: [LIST]

ACTION ITEMS:
[List any items needing attention]

View all data: [SHEET LINK]

- Automation Bot
```

---

## Tips for Using Templates

1. **Personalize**: Always replace [BRACKETS] with actual values
2. **Proofread**: Double-check names and codes before sending
3. **Timing**: Send welcome emails within 24 hours of approval
4. **Tone**: Keep it friendly but professional
5. **Follow-up**: If no response in 3-5 days, send one follow-up
